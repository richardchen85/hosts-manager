    ...
    return this.state.modals.map((options, index) => {
      ...
      if (options.buttons) {
        buttons = Object.keys(options.buttons).map((button, index) => {
          ...
          return (
            <button className="dialog-btn" data-status={status} key={index} onClick={handle}>
              {button}
            </button>
          );
        });
      ...
      return (
        ...
        >
          <div className="dialog" style={style}>
            ...
            {header}
            <div className="cont">{options.content}</div>
            {buttons.length > 0 && <div className="footer">{buttons}</div>}
          </div>
        </div>
      );
    });
  ...
  render() {
    ...
    return (
      <div className={className} style={{ zIndex: Z_INDEX }}>
        <div
          className={'mask' + (len > 0 ? ' active' : '')}
          style={{ zIndex: Z_INDEX + len - 1 }}
        />
        {this.renderModals()}
      </div>
    );
