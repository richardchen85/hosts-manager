...
export default function (state = Map(), action) {
  switch (action.type) {
    ...
    case types.PROJECT_EDIT:
      return state.setIn(['projects', action.projIndex, 'projectName'], action.projectName);
    case types.PROJECT_ORDER:
      return (() => {
        ...
        return state
          .setIn(['projects', projIndex], before)
          .setIn(['projects', projIndex - 1], curr);
      })();
    case types.GROUP_ADD:
      let keyPath = ['projects', action.projIndex, 'groups'];
      return state.setIn(keyPath, state.getIn(keyPath).push(action.group));
    case types.GROUP_DELETE:
      return state.deleteIn(['projects', action.projIndex, 'groups', action.groupIndex]);
    ...
    case types.GROUP_DE_EDIT:
      return state.setIn(['projects', action.projIndex, 'groups', action.groupIndex, 'status'], '');
